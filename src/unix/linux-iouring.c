...
#define munmap_checked(addr, size) \
  do { if (munmap((addr), (size))) abort(); } while (0)
...
int uv__platform_fs_read(uv_loop_t* loop,
                         ...
                         int64_t off,
                         uv_fs_cb cb) {
  return uv__io_uring_fs_work(UV__IORING_OP_READV,
...
int uv__platform_fs_write(uv_loop_t* loop,
                          ...
                          int64_t off,
                          uv_fs_cb cb) {
  return uv__io_uring_fs_work(UV__IORING_OP_WRITEV,
                              loop, req, file, bufs, nbufs, off, cb);
}
...
int uv__platform_fs_fsync(uv_loop_t* loop,
                          ...
                          uv_os_fd_t file,
                          uv_fs_cb cb) {
  return uv__io_uring_fs_work(UV__IORING_OP_FSYNC,
                              loop, req, file, NULL, 0, 0, cb);
}
